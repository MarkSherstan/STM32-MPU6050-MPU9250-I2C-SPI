...
uint8_t MPUXX50::begin()
{
    ...
    if ((check == WHO_AM_I_9250_ANS) || (check == WHO_AM_I_6050_ANS))
    {
        ...
        writeAccFullScaleRange(I2Cx, aScale);
        writeGyroFullScaleRange(I2Cx, gScale);
...
void MPUXX50::writeAccFullScaleRange(uint8_t aScale)
{
    // Variable init
    uint8_t select;
...
void MPUXX50::writeGyroFullScaleRange(uint8_t gScale)
{
    // Variable init
    uint8_t select;
